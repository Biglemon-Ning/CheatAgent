...
class TestDataset(Dataset):
    def __init__(self, args, dataset, task, data_num=None):
        super().__init__()
        ...
        if data_num == None:
            ...
            self.data_samples = self.load_test()[:data_num]
    ...
    def construct_sentence(self):
        ...
        for i in range(len(self.data_samples)):
            ...
            self.data['prompt'].append(prompt)
            self.data['output'].append(prompt['Output'].format(**datapoint))
            self.data['raw_data'].append(datapoint)
    ...
    def get_item_filtered(self, idx):
        ...
        return {'user_idx': self.user2id[self.data_samples[idx]['user_id']],
               ...
               'raw_data': self.data['raw_data'][idx],
