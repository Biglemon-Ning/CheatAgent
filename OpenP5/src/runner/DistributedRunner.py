...
import numpy as np
import random
from runner.Attack import adversarial_attack
...
class DistributedRunner(SingleRunner):
    ...
    def __init__(self, model, tokenizer, train_loader, valid_loader, device, args, rank):
        ...
        self.attack = adversarial_attack(self.model, tokenizer, train_loader, valid_loader, device, args, rank)
    
    def get_testloader(self, data_num=None):
        self.testloaders = []
        ...
        for dataset in datasets:
            for task in tasks:
                testdata = TestDataset(self.args, dataset, task, data_num)
        ...
        for loader in self.testloaders:
            if self.test_filtered > 0:
                if self.test_filtered_batch > 0:
                    self.args.test_mode = 0
                    logging.info(f'Test dataset task filtered batch')
                    self.test_dataset_task_filtered_batch(loader)
                else:
                    assert self.args.eval_batch_size == 1
                    self.args.test_mode = 1
                    logging.info(f'Test dataset task filtered')
                    self.test_dataset_task_filtered(loader)
            else:
                self.args.test_mode = 2
                logging.info(f'Test dataset task')
    ...
    def test_dataset_task_filtered(self, testloader):
        ...
        for batch in tqdm(testloader):
            ...
            adversarial_batch, cos_sim, norm_1, norm_2 = self.attack(batch, testloader)
            # print(self.tokenizer.batch_decode(adversarial_batch[0], skip_special_tokens=True))

            with torch.no_grad():
                input_ids = adversarial_batch[0].to(self.device)
                attn = adversarial_batch[1].to(self.device)
                whole_input_ids = adversarial_batch[2].to(self.device)
                output_ids = batch[3].to(self.device)
                output_attention = batch[4].to(self.device)
        ...
        dist.all_reduce(cos_sim_total, op=dist.ReduceOp.SUM)
        dist.all_reduce(norm_1_total, op=dist.ReduceOp.SUM)
        dist.all_reduce(norm_2_total, op=dist.ReduceOp.SUM)
            
        metrics_res /= test_total

        cos_sim_total /= test_total
        norm_1_total /= test_total
        norm_2_total /= test_total

        print(f'Cos: {cos_sim_total}  Norm 1: {norm_1_total}  Norm 2: {norm_2_total}')
