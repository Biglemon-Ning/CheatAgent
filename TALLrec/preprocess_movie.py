...
sequential_interaction_list = sequential_interaction_list[-1000209:] # 10000 records
# random.shuffle(sequential_interaction_list)

import csv
# save the csv file for baselines
with open('./data/ml1m_raw/train.csv', 'w') as f:
    writer = csv.writer(f)
...
with open('./data/ml1m_raw/valid.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['user_id', 'history_movie_id', 'history_rating', 'movie_id', 'rating', 'timestamp'])
...
with open('./data/ml1m_raw/test.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['user_id', 'history_movie_id', 'history_rating', 'movie_id', 'rating', 'timestamp'])
    writer.writerows(sequential_interaction_list[int(len(sequential_interaction_list)*0.9):])
with open('./data/ml1m_raw/test_filtered.csv', 'w') as f:
    writer = csv.writer(f)
    writer.writerow(['user_id', 'history_movie_id', 'history_rating', 'movie_id', 'rating', 'timestamp'])
    interaction_list = sequential_interaction_list[int(len(sequential_interaction_list)*0.9):]
    random.shuffle(interaction_list)
    interaction_list = interaction_list[:10000]
...
def csv_to_json(input_path, output_path):
    ...
    for index, row in data.iterrows():
        ...
        unpreference = []
        for i in range(L):
            if int(row['history_movie_id'][i]) - 1 >= len(movie_names):
                continue
        ...
        if int(row['movie_id']) - 1 >= len(movie_names):
            continue
        ...
        json_list.append({
            "instruction": f"Given the user's preference and unpreference, identify whether the user will like the target movie by answering \"Yes.\" or \"No.\".",
...
# csv_to_json('./data/ml1m_raw/train.csv', './data/ML1M/train.json')
# csv_to_json('./data/ml1m_raw/valid.csv', './data/ML1M/valid.json')
# csv_to_json('./data/ml1m_raw/test.csv',  './data/ML1M/test.json')
csv_to_json('./data/ml1m_raw/test_filtered.csv',  './data/ML1M/test_filtered.json')